...
export class DataClassificationSystemService {
  ...
  async classifyData(
    ...
    try {
      ...
      const ruleBasedResult =
        await this.performRuleBasedClassification(dataItem);
      ...
      const vietnameseHealthcareResult =
        await this.performVietnameseHealthcareClassification(dataItem);

      // Step 4: International standards classification
      const internationalResult =
        await this.performInternationalClassification(dataItem);
      ...
      const handlingRequirements =
        await this.determineHandlingRequirements(finalClassification);

      // Step 7: Generate labels and metadata
      const labels =
        await this.generateClassificationLabels(finalClassification);

      const result: DataClassificationResult = {
        data_item_id: dataItem.id,
        classification_date: new Date(),
        final_classification: finalClassification,
        classification_confidence: this.calculateConfidence([
          ruleBasedResult,
          mlBasedResult,
          vietnameseHealthcareResult,
          internationalResult,
        ...
        sensitivity_score:
          await this.calculateSensitivityScore(finalClassification),
        compliance_frameworks:
          await this.identifyApplicableFrameworks(finalClassification),
        recommended_controls:
          await this.recommendSecurityControls(finalClassification),
        retention_requirements:
          await this.determineRetentionRequirements(finalClassification),
        access_restrictions:
          await this.determineAccessRestrictions(finalClassification),
      };
      ...
      const classifiedItem: ClassifiedDataItem = {
        ...
        review_schedule:
          await this.determineReviewSchedule(finalClassification),
  ...
  private async performVietnameseHealthcareClassification(
    ...
    const healthcareCategories =
      await this.analyzeVietnameseHealthcareCategories(dataItem);
    ...
    if (healthcareCategories.personalMedicalInfo) {
      ...
      classificationResult.reasoning.push(
        "Chứa thông tin y tế cá nhân - Personal medical information detected",
      );
      classificationResult.applicable_regulations.push(
        "Luật Khám chữa bệnh",
    ...
    if (healthcareCategories.healthInsuranceInfo) {
      ...
      classificationResult.confidence = Math.max(
        classificationResult.confidence,
        0.9,
      );
      classificationResult.reasoning.push(
        "Chứa thông tin bảo hiểm y tế - Health insurance information detected",
      );
      classificationResult.applicable_regulations.push(
        "Luật Bảo hiểm y tế",
        "Quy định BHXH",
      );
      classificationResult.data_types.push(
    ...
    if (healthcareCategories.biometricInfo) {
      ...
      classificationResult.reasoning.push(
        "Chứa dữ liệu sinh trắc học - Biometric data detected",
      );
      classificationResult.applicable_regulations.push(
    ...
    if (healthcareCategories.medicalRecords) {
      ...
      classificationResult.reasoning.push(
        "Chứa hồ sơ bệnh án - Medical records detected",
      );
      classificationResult.applicable_regulations.push(
    ...
    if (healthcareCategories.medicalResearchData) {
      ...
      classificationResult.reasoning.push(
        "Chứa dữ liệu nghiên cứu y học - Medical research data detected",
      );
      classificationResult.applicable_regulations.push(
        "Quy định nghiên cứu khoa học y học",
      );
    }
  ...
  private async performInternationalClassification(
    ...
    const gdprSpecialCategories =
      await this.analyzeGDPRSpecialCategories(dataItem);
    if (gdprSpecialCategories.detected) {
      ...
      internationalResult.reasoning.push(
        "GDPR Article 9 special categories detected",
      );
    ...
    if (hipaaAnalysis.isPHI) {
      ...
      internationalResult.confidence = Math.max(
        internationalResult.confidence,
        0.9,
      );
      internationalResult.reasoning.push(
        "HIPAA Protected Health Information detected",
      );
      internationalResult.applicable_regulations.push(
        "HIPAA Privacy Rule",
        "HIPAA Security Rule",
      );
    ...
    const isoClassification =
      await this.performISO27001Classification(dataItem);
    if (isoClassification.classification !== DataClassification.PUBLIC) {
      ...
      internationalResult.confidence = Math.max(
        internationalResult.confidence,
        isoClassification.confidence,
      );
      internationalResult.reasoning.push(
        `ISO 27001 classification: ${isoClassification.rationale}`,
      );
  ...
  private async performMLBasedClassification(
    ...
    if (healthClassifier) {
      ...
      if (healthPrediction.confidence > 0.8) {
        ...
        mlResult.reasoning.push(
          `ML health classifier: ${healthPrediction.confidence * 100}% confidence`,
        );
    ...
    if (piiClassifier) {
      ...
      if (piiPrediction.confidence > 0.7) {
        ...
        mlResult.confidence = Math.max(
          mlResult.confidence,
          piiPrediction.confidence,
        );
        mlResult.reasoning.push(
          `ML PII classifier: ${piiPrediction.confidence * 100}% confidence`,
        );
    ...
    if (financialClassifier) {
      ...
      if (financialPrediction.confidence > 0.75) {
        ...
        mlResult.confidence = Math.max(
          mlResult.confidence,
          financialPrediction.confidence,
        );
        mlResult.reasoning.push(
          `ML financial classifier: ${financialPrediction.confidence * 100}% confidence`,
        );
    ...
    if (biometricClassifier) {
      ...
      if (biometricPrediction.confidence > 0.85) {
        ...
        mlResult.confidence = Math.max(
          mlResult.confidence,
          biometricPrediction.confidence,
        );
        mlResult.reasoning.push(
          `ML biometric classifier: ${biometricPrediction.confidence * 100}% confidence`,
        );
  ...
  private async performRuleBasedClassification(
    ...
    for (const rule of rules) {
      ...
      if (matches.isMatch) {
        ...
        ruleResult.confidence = Math.max(
          ruleResult.confidence,
          matches.confidence,
        );
        ruleResult.reasoning.push(
          `Rule match: ${rule.name} - ${matches.rationale}`,
        );
  ...
  async discoverAndClassifyDataSources(
    ...
    try {
      ...
      if (discoveryRequest.includeFileSystems) {
        const fileSources =
          await this.discoverFileSystemSources(discoveryRequest);
      ...
      if (discoveryRequest.includeCloudStorage) {
        const cloudSources =
          await this.discoverCloudStorageSources(discoveryRequest);
      ...
      const result: DataDiscoveryResult = {
        ...
        summary: await this.generateDiscoverySummary(
          discoveredSources,
          classificationResults,
        ),
        recommendations: await this.generateDiscoveryRecommendations(
          classificationResults,
        ),
  ...
  async calculateDataSensitivity(
    ...
    try {
      ...
      const businessFactor =
        await this.assessBusinessImpactSensitivity(dataItem);
      sensitivityFactors.push(businessFactor);

      const overallScore =
        this.calculateOverallSensitivityScore(sensitivityFactors);
      ...
      const result: SensitivityScore = {
        ...
        contributing_elements: await this.identifyContributingElements(
          dataItem,
          sensitivityFactors,
        ),
        recommendations: await this.generateSensitivityRecommendations(
          overallScore,
          sensitivityFactors,
        ),
        handling_requirements:
          await this.determineSensitivityBasedHandling(overallScore),
      };
      ...
      return {
        success: false,
        error:
          error instanceof Error ? error.message : "Sensitivity scoring failed",
      };
    }
  }
  ...
  private initializeVietnameseClassificationSchemes(): void {
      ...
      levels: [
        {
          ...
          handling_requirements: [
            ...
            "Không yêu cầu bảo mật đặc biệt",
          ],
        },
        {
          ...
          criteria: [
            "Báo cáo nội bộ",
            "Thống kê không định danh",
            "Dữ liệu tổng hợp",
          ],
          handling_requirements: [
            "Chỉ chia sẻ trong nội bộ",
        ...
        },
        {
          ...
          criteria: [
            ...
            "Dữ liệu bệnh nhân có định danh",
            "Thông tin tài chính",
          ],
          handling_requirements: [
            "Mã hóa khi truyền",
    ...
    this.classificationSchemes.set(
      "vietnamese_healthcare",
      ...
      levels: [
        {
          ...
          criteria: [
            "Không ảnh hưởng an ninh quốc gia",
            "Không chứa thông tin cá nhân",
          ],
    ...
    this.classificationSchemes.set(
      "vietnamese_cybersecurity",
      vietnameseCybersecurity,
    );
  }
      ...
      levels: [
        {
          ...
          description:
            "Data that does not relate to identified or identifiable individuals",
          criteria: [
            "Anonymous data",
        ...
        },
        {
          ...
          handling_requirements: [
            "Explicit consent or specific legal basis",
            "Enhanced protection",
          ],
        },
      ],
      ...
      levels: [
        {
          ...
          criteria: [
            "No adverse effect if disclosed",
        ...
        },
        {
          ...
          criteria: [
            "Minor adverse effect if disclosed",
        ...
        },
        {
          ...
          criteria: [
            "Moderate adverse effect if disclosed",
        ...
        },
        {
          ...
          name: "Restricted",
          description:
            "Highly sensitive information requiring highest protection",
          criteria: [
            "Severe adverse effect if disclosed",
  ...
  // Helper methods
  private async analyzeVietnameseHealthcareCategories(
    dataItem: DataClassificationRequest,
  ): Promise<VietnameseHealthcareCategories> {
    return {
  ...
  private countHealthTerms(content: string): number {
    ...
    return healthTerms.filter((term) => content.toLowerCase().includes(term))
      .length;
  }
  ...
  private calculatePIIScore(content: string): number {
    ...
    // Vietnamese names
    if (
      /\b(Nguyễn|Trần|Lê|Phạm|Hoàng|Huỳnh|Phan|Vũ|Võ|Đ��ng|Bùi|Đỗ|Hồ|Ngô|Dương|Lý)\b/.test(
        content,
      )
    )
      score += 25;
  ...
  private async resolveClassificationConflicts(
