...
export class KeyManagementService extends EventEmitter {
  ...
  async rotateKey(
    ...
    try {
      ...
      await this.logKeyOperation("rotate", keyId, userId, false, {
        error: error.message,
      });
      throw error;
    }
  ...
  async revokeKey(
    ...
    try {
      ...
      await this.logKeyOperation("revoke", keyId, userId, false, {
        error: error.message,
      });
      throw error;
    }
  ...
  async backupKeys(vaultId?: string): Promise<{
    ...
    try {
      const keysToBackup = vaultId
        ...
        : Array.from(this.managedKeys.values()).filter(
  ...
  async recoverKey(
    ...
    try {
      ...
      throw new Error(
        "Key recovery failed - no valid recovery method available",
      );
    } catch (error) {
      await this.logKeyOperation("restore", keyId, userId, false, {
        error: error.message,
      });
      throw error;
    }
  ...
  async isHealthy(): Promise<boolean> {
    return (
      ...
      Array.from(this.keyVaults.values()).some(
        (vault) => vault.high_availability,
      )
    );
  }

  // Private helper methods
  private selectOptimalVault(
    algorithm: string,
    purpose: string,
  ): KeyVault | null {
    const vaults = Array.from(this.keyVaults.values())
      ...
      .sort((a, b) => {
        ...
        return (
          securityLevelPriority[b.security_level] -
          securityLevelPriority[a.security_level]
        );
      });
  ...
  private async validateKeyCreation(
    algorithm: string,
    purpose: string,
    keyLength: number,
  ): Promise<void> {
    const applicablePolicies = this.keyPolicies.filter(
      (policy) => policy.enabled && policy.key_types.includes(purpose),
    );

    for (const policy of applicablePolicies) {
      if (keyLength < policy.minimum_key_length) {
        throw new Error(
          `Key length ${keyLength} below minimum ${policy.minimum_key_length} for policy ${policy.name}`,
        );
      }
    }
  }
  ...
  private determineRotationPolicy(
    purpose: string,
  ): EncryptionKey["rotation_policy"] {
    switch (purpose) {
      case "key_encryption":
      case "signing":
        return "quarterly";
  ...
  private async scheduleMigration(
    oldKeyId: string,
    newKeyId: string,
  ): Promise<void> {
  ...
  private async canRevokeKey(
    ...
    if (key.purpose === "key_encryption") {
      return {
        allowed: false,
        reason: "Cannot revoke key encryption keys without migration",
      };
    }
  ...
  private async requestRecoveryApproval(
    keyId: string,
    userId: string,
    justification: string,
  ): Promise<void> {
  ...
  private async recoverFromEscrow(
    keyId: string,
    userId: string,
  ): Promise<string> {
    const escrow = this.keyEscrows.get(keyId);
  ...
  private async scheduleKeyOperations(): Promise<void> {
    ...
    setInterval(
      async () => {
        const keysNeedingRotation = Array.from(
          this.managedKeys.values(),
  ...
  private async validateKeyAgainstPolicies(
    key: EncryptionKey,
  ): Promise<boolean> {
    const applicablePolicies = this.keyPolicies.filter(
      (policy) => policy.enabled && policy.key_types.includes(key.purpose),
    );
