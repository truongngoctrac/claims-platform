...
export class MultiFactorAuthService extends EventEmitter {
  ...
  async setupMFA(userId: string, type: MFAMethod['type'], details: any): Promise<{ secret?: string; qr_code?: string; setup_complete: boolean }> {
    ...
    if (type === 'totp') {
      ...
      return { secret, qr_code: this.generateQRCode(secret), setup_complete: false };
    }

    return { setup_complete: true };
  }

  async verifyMFA(userId: string, code: string, methodType?: string): Promise<{ verified: boolean; method_used: string }> {
    const userMethods = this.mfaMethods.get(userId) || [];
  ...
  async createChallenge(userId: string, methodType: string): Promise<{ challenge_id: string; delivery_method: string }> {
    const userMethods = this.mfaMethods.get(userId) || [];
  ...
  private generateBackupCodes(): string[] {
    return Array.from({ length: 10 }, () => Math.random().toString(36).substring(2, 10));
  }
  ...
  private generateChallengeCode(methodType: string): string {
    return methodType === 'sms' ? Math.floor(100000 + Math.random() * 900000).toString() : Math.random().toString(36).substring(2, 8);
  }

  private async validateCode(method: MFAMethod, code: string): Promise<boolean> {
    if (method.type === 'totp') {
