...
interface IntrusionEvent {
  ...
  intrusion_type: 'sql_injection' | 'xss' | 'command_injection' | 'path_traversal' | 'brute_force' | 'ddos' | 'malware' | 'unauthorized_access';
  severity: 'low' | 'medium' | 'high' | 'critical';
  action_taken: string;
...
export class IntrusionPreventionService extends EventEmitter {
  ...
  private rateLimiters: Map<string, { count: number; lastReset: Date; blocked: boolean }> = new Map();
  ...
  private async checkRateLimit(request: any): Promise<{
    ...
    if (limiter.count > rateLimit.requests_per_minute) {
      ...
      if (limiter.count > rateLimit.requests_per_minute * 3) {
        await this.blockEntity('ip_address', request.source_ip, 'Rate limit exceeded', 'medium', 'ips_rate_limit');
      }
  ...
  private async evaluateRules(request: any): Promise<{
    ...
    for (const rule of this.ipsRules.filter(r => r.enabled).sort((a, b) => b.priority - a.priority)) {
    ...
    const allowed = !triggeredRules.length || 
  ...
  private async evaluateRule(rule: IPSRule, request: any): Promise<boolean> {
    ...
    if (conditions.destination_ip && conditions.destination_ip.length > 0) {
      if (!this.matchIPPatterns(request.destination_ip, conditions.destination_ip)) {
        return false;
      }
    }

    // Port checks
    if (conditions.source_port && !conditions.source_port.includes(request.source_port)) {
      return false;
    }
    if (conditions.destination_port && !conditions.destination_port.includes(request.destination_port)) {
      return false;
    }

    // Protocol check
    if (conditions.protocol && !conditions.protocol.includes(request.protocol)) {
      return false;
    }
    ...
    if (conditions.header_patterns) {
      for (const [headerName, pattern] of Object.entries(conditions.header_patterns)) {
  ...
  private async executePreventionAction(request: any, evaluation: any): Promise<void> {
    ...
    switch (evaluation.action_taken) {
      case 'block':
        await this.blockEntity('ip_address', request.source_ip, evaluation.reason || 'IPS rule triggered', 'high', evaluation.triggered_rules[0]);
        break;
      case 'quarantine':
        await this.quarantineEntity(request.source_ip, evaluation.reason || 'Quarantined by IPS');
        break;
      case 'drop':
  ...
  async unblockEntity(blockId: string): Promise<boolean> {
    const index = this.blockedEntities.findIndex(entity => entity.id === blockId);
  ...
  async addIPSRule(rule: Omit<IPSRule, 'id' | 'created_at' | 'updated_at' | 'match_count' | 'last_triggered'>): Promise<string> {
  ...
  async updateIPSRule(ruleId: string, updates: Partial<IPSRule>): Promise<boolean> {
  ...
  async getBlockedEntities(): Promise<BlockedEntity[]> {
    ...
    const activeBlocks = this.blockedEntities.filter(entity => 
      !entity.expires_at || entity.expires_at > new Date()
    );
    
    return activeBlocks.sort((a, b) => b.blocked_at.getTime() - a.blocked_at.getTime());
  }

  async isHealthy(): Promise<boolean> {
    return this.isActive &&
           this.ipsRules.filter(r => r.enabled).length > 0;
  }
  ...
  private determineIntrusionType(triggeredRules: string[]): IntrusionEvent['intrusion_type'] {
    // Map rule IDs to intrusion types
    if (triggeredRules.some(rule => rule.includes('sql'))) return 'sql_injection';
    if (triggeredRules.some(rule => rule.includes('xss'))) return 'xss';
    if (triggeredRules.some(rule => rule.includes('command'))) return 'command_injection';
    if (triggeredRules.some(rule => rule.includes('path'))) return 'path_traversal';
    if (triggeredRules.some(rule => rule.includes('brute'))) return 'brute_force';
    if (triggeredRules.some(rule => rule.includes('ddos'))) return 'ddos';
    return 'unauthorized_access';
  }

  private calculateSeverity(triggeredRules: string[]): IntrusionEvent['severity'] {
  ...
  private getRateLimitForEndpoint(url: string): { requests_per_minute: number; bytes_per_second: number } {
    // Different rate limits for different endpoints
    if (url.includes('/api/auth')) return { requests_per_minute: 10, bytes_per_second: 1024 };
    if (url.includes('/api/')) return { requests_per_minute: 100, bytes_per_second: 10240 };
    return { requests_per_minute: 200, bytes_per_second: 20480 };
  }

  private async quarantineEntity(sourceIp: string, reason: string): Promise<void> {
    // Implement quarantine logic - restrict access but don't completely block
    this.emit('entity_quarantined', { ip: sourceIp, reason, timestamp: new Date() });
  }
  ...
  private calculateIntrusionTrends(): any {
    ...
    return {
      ...
      blocked_percentage: last24h.length > 0 ? (last24h.filter(e => e.blocked).length / last24h.length) * 100 : 0,
  ...
  private async performMaintenance(): Promise<void> {
    ...
    this.intrusionEvents = this.intrusionEvents.filter(event => event.timestamp > oneWeekAgo);
