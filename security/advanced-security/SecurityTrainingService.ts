...
export class SecurityTrainingService extends EventEmitter {
  ...
  ): Promise<{ assignment_id: string; estimated_duration: number; requirements: string[] }> {
  ...
  async startTraining(assignmentId: string, userContext: { ip_address: string; device_info: string }): Promise<{
  ...
  async submitTrainingResults(
    assignmentId: string,
    answers: number[],
    sessionContext: { session_id: string; ip_address: string; device_info: string; start_time: Date }
  ): Promise<{
    passed: boolean;
    score: number;
    ...
    const timeSpent = Math.round((endTime.getTime() - sessionContext.start_time.getTime()) / 1000 / 60);

    // Grade the answers
    const gradingResult = this.gradeAnswers(module.quiz_questions, answers);
    const passed = gradingResult.score >= module.completion_criteria.minimum_score;
    ...
    if (passed) {
      ...
      const canRetry = assignment.attempts < module.completion_criteria.retries_allowed;
      assignment.status = canRetry ? 'assigned' : 'failed';
    }

    const canRetry = assignment.attempts < module.completion_criteria.retries_allowed && !passed;
  ...
  async getTrainingProgress(userId: string): Promise<{
    ...
    const userAssignments = Array.from(this.assignments.values()).filter(a => a.user_id === userId);
    const now = new Date();
    ...
    const overdue = userAssignments.filter(a => a.due_date < now && a.status !== 'completed').length;
  ...
  async getOrganizationStatistics(): Promise<{
    ...
    by_module: Record<string, { assigned: number; completed: number; rate: number }>;
    by_department: Record<string, { assigned: number; completed: number; rate: number }>;
    ...
    const now = new Date();
    const overdueCount = allAssignments.filter(a => a.due_date < now && a.status !== 'completed').length;

    const byModule: Record<string, { assigned: number; completed: number; rate: number }> = {};
    ...
    Object.keys(byModule).forEach(moduleId => {
      ...
      stats.rate = stats.assigned > 0 ? (stats.completed / stats.assigned) * 100 : 0;
    });
    ...
    return {
      ...
      overall_completion_rate: allAssignments.length > 0 ? (completedAssignments.length / allAssignments.length) * 100 : 0,
      by_module: byModule,
  ...
  // Private methods
  private gradeAnswers(questions: SecurityTrainingModule['quiz_questions'], answers: number[]): {
    score: number;
  ...
  private async generateCertificate(assignment: TrainingAssignment, module: SecurityTrainingModule, result: TrainingResult): Promise<void> {
    const certificate = {
      id: `cert_${Date.now()}`,
      user_id: assignment.user_id,
      module_id: assignment.module_id,
  ...
  private async sendAssignmentNotification(assignment: TrainingAssignment, module: SecurityTrainingModule): Promise<void> {
    ...
    const modules: SecurityTrainingModule[] = [
      {
        ...
        description: 'Learn to identify and respond to phishing attacks targeting healthcare organizations',
        category: 'phishing',
        difficulty: 'beginner',
        duration_minutes: 30,
        ...
        quiz_questions: [
          {
            ...
            options: ['Click the link to verify', 'Delete it immediately', 'Report it to IT security', 'Forward it to colleagues'],
            correct_answer: 2,
            explanation: 'Always report suspicious emails to IT security first before taking any other action.'
          },
          {
            question: 'Which of these is a common sign of a phishing email?',
            options: ['Professional formatting', 'Urgent language requiring immediate action', 'Personalized greeting', 'Company logo'],
            correct_answer: 1,
            explanation: 'Phishing emails often use urgent language to pressure victims into quick decisions.'
          },
          {
            question: 'What should you do before clicking any link in an email?',
            options: ['Click it quickly', 'Hover over it to see the actual URL', 'Forward the email first', 'Print the email'],
            correct_answer: 1,
      ...
      },
      {
        ...
        description: 'Understanding data protection requirements for healthcare organizations',
        ...
        quiz_questions: [
          {
            question: 'Under Vietnamese healthcare law, what is required when sharing patient data?',
            options: ['Patient consent', 'Manager approval', 'Written request', 'Email confirmation'],
            correct_answer: 0,
            explanation: 'Patient consent is required before sharing any healthcare data under Vietnamese law.'
          },
          {
            question: 'How long should healthcare records be retained?',
            options: ['1 year', '5 years', '7 years', '10 years'],
            correct_answer: 2,
            explanation: 'Vietnamese healthcare regulations require medical records to be retained for at least 7 years.'
          }
        ],
        created_at: new Date(),
        updated_at: new Date(),
      ...
      },
      {
        ...
        description: 'Best practices for creating and managing secure passwords in healthcare environments',
        ...
        quiz_questions: [
          {
            ...
            options: ['At least 8 characters with mixed case and numbers', 'Simple word easy to remember', 'Your birthday', 'The hospital name'],
            correct_answer: 0,
            explanation: 'Strong passwords should be at least 8 characters long with mixed case letters, numbers, and special characters.'
          }
        ],
        created_at: new Date(),
        updated_at: new Date(),
    ...
    const campaigns: TrainingCampaign[] = [
      {
        ...
        description: 'Mandatory annual security training for all healthcare staff',
        target_roles: ['all_staff'],
        module_ids: ['healthcare_phishing_awareness', 'password_security_healthcare'],
        start_date: new Date(),
