...
export class DataLossPreventionService extends EventEmitter {
  ...
  async scanAndProtectData(request: {
    ...
    try {
      ...
      const policyEvaluation = await this.evaluateDLPPolicies(
        request,
        classification,
        sensitiveDataFound,
        riskScore,
      ...
      if (policyEvaluation.violations.length > 0) {
        ...
        const protectionResult = await this.executeProtectiveActions(
          request,
          policyEvaluation,
        );
      ...
      const autoProtection = await this.applyAutomaticProtection(
        request,
        classification,
      );
  ...
  private async calculateRiskScore(
    ...
    if (
      request.metadata.file_size &&
      request.metadata.file_size > 10 * 1024 * 1024
    ) {
      // 10MB
      riskScore += 2;
    }
  ...
  private async evaluateDLPPolicies(
    ...
    for (const policy of this.dlpPolicies
      .filter((p) => p.enabled)
      .sort((a, b) => b.priority - a.priority)) {
      const violates = await this.evaluatePolicy(
        policy,
        request,
        classification,
        sensitiveDataFound,
      ...
      if (violates) {
        ...
        if (
          this.getActionPriority(policy.actions.primary) >
          this.getActionPriority(highestPriorityAction)
        ) {
  ...
  private async evaluatePolicy(
    ...
    if (policy.data_types.length > 0) {
      const hasMatchingDataType = policy.data_types.some(
        (type) =>
    ...
    const dataString =
      typeof request.data === "string" ? request.data : request.data.toString();
    if (policy.conditions.contains_patterns.length > 0) {
      const hasMatchingPattern = policy.conditions.contains_patterns.some(
        (pattern) => {
          try {
    ...
    if (
      policy.conditions.recipient_domains &&
      request.metadata.recipient_email
    ) {
      const domain = request.metadata.recipient_email.split("@")[1];
    ...
    if (policy.conditions.location_restrictions) {
      if (
        !policy.conditions.location_restrictions.includes(
  ...
  private async executeProtectiveActions(
    request: any,
    ...
    switch (primaryAction) {
      ...
      case "watermark":
        const watermarkedData = await this.watermarkData(
          request.data,
          request.user_email,
        );
        return {
  ...
  async getDLPStatistics(): Promise<{
    ...
    this.violations.forEach((violation) => {
      byPolicy[violation.policy_name] =
        (byPolicy[violation.policy_name] || 0) + 1;
  ...
  async addDLPPolicy(
    policy: Omit<
      DLPPolicy,
      "id" | "created_at" | "updated_at" | "violation_count" | "last_triggered"
    >,
  ): Promise<string> {
  ...
  private isWithinTimeRestrictions(restrictions: {
    start: string;
    end: string;
    days: string[];
  }): boolean {
  ...
  private async applyAutomaticProtection(
    request: any,
    classification: DataClassification["classification"],
  ): Promise<{
    encrypted_data?: string;
    ...
    if (classification === "confidential" && request.action === "email") {
      result.watermarked_data = await this.watermarkData(
        request.data,
        request.user_email,
      );
    }
    ...
    this.patientDataScanners = [
      {
        ...
        patterns: {
          ...
          medical_record_number: [/MRN\d{8,12}/i, /\b\d{10,12}\b/],
          health_insurance_number: [
            /BHYT\d{10,15}/i,
            /\d{2}\.\d{2}\.\d{3}\.\d{2}\.\d{5}/,
          ],
          social_security_number: [/\b\d{3}-\d{2}-\d{4}\b/, /\b\d{9}\b/],
          phone_number: [/\+84\d{9,10}/, /0\d{9,10}/, /\b\d{3}-\d{3}-\d{4}\b/],
