...
export class SecureCommunicationService extends EventEmitter {
  ...
  getSecurityHeaders(): SecurityHeaders {
    return {
      ...
      "Content-Security-Policy":
        "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self'; connect-src 'self'; frame-src 'none'; object-src 'none'; base-uri 'self'",
  ...
  async encryptMessage(
    ...
    try {
      ...
      const cipher = crypto.createCipherGCM(
        this.messageEncryption.algorithm.toLowerCase(),
        ephemeralKey,
        iv,
      );
      ...
      const messageHash = crypto
        .createHash(this.messageEncryption.hashFunction.toLowerCase())
      ...
      this.metrics.encryptedDataTransferred += Buffer.byteLength(
        message,
        "utf8",
      );
  ...
  async decryptMessage(
    ...
    try {
      ...
      const messageHash = crypto
        ...
        .update(
          secureMessage.messageId +
            secureMessage.timestamp.toISOString() +
      ...
      const encryptedData = secureMessage.encryptedPayload.slice(
        0,
        -authTagLength,
      );
      const authTag = Buffer.from(
        secureMessage.encryptedPayload.slice(-authTagLength),
        "hex",
      );
      ...
      this.emit("messageDecryptionError", {
    ...
    if (isSecure) {
      ...
      if (handshakeTime) {
        this.metrics.averageHandshakeTime =
          (this.metrics.averageHandshakeTime *
            (this.metrics.secureConnections - 1) +
            handshakeTime) /
          this.metrics.secureConnections;
      }
    } else {
