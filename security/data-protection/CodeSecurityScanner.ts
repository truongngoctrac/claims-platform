...
export interface ScanType {
  type: 'sast' | 'dependency' | 'secret' | 'license' | 'infrastructure' | 'custom';
  enabled: boolean;
  configuration?: any;
}
...
export interface SecurityRule {
  ...
  name: string;
  description: string;
  category: 'injection' | 'xss' | 'authentication' | 'authorization' | 'cryptography' | 'configuration' | 'custom';
...
export class CodeSecurityScanner extends EventEmitter {
  ...
  private async loadSecurityRules(): Promise<void> {
    const rules: SecurityRule[] = [
      // SQL Injection Rules
      {
        ...
        messageTemplate: 'SQL query constructed using string concatenation. Use parameterized queries instead.',
      ...
      },
      {
        ...
        description: 'SQL query directly incorporating user input without sanitization',
        category: 'injection',
        severity: 'critical',
        pattern: /\.query\(['"`][^'"`]*\$\{[^}]*\}[^'"`]*['"`]\)/gi,
        messageTemplate: 'SQL query contains unsanitized user input. Use parameterized queries.',
        remediation: 'Always use parameterized queries when incorporating user input into SQL statements.',
        cweId: 'CWE-89',
        owaspCategory: 'A03:2021 - Injection'
      },
      ...
      // XSS Rules
      {
        ...
        description: 'Using innerHTML with user data may lead to XSS vulnerabilities',
        category: 'xss',
        severity: 'high',
        pattern: /\.innerHTML\s*=\s*(?!['"`][^'"`]*['"`])[^;]+/gi,
        messageTemplate: 'Setting innerHTML with user data may cause XSS. Use textContent or sanitize input.',
        remediation: 'Use textContent for plain text or properly sanitize HTML content using a trusted library.',
        cweId: 'CWE-79',
        owaspCategory: 'A03:2021 - Injection'
      },
      ...
      // Authentication Rules
      {
        ...
        severity: 'critical',
        pattern: /(password|passwd|pwd|key|secret|token)\s*[:=]\s*['"`][^'"`\s]{8,}['"`]/gi,
        messageTemplate: 'Hardcoded credentials detected. Use environment variables or secure vaults.',
        remediation: 'Store credentials in environment variables or secure credential management systems.',
        cweId: 'CWE-798',
      ...
      // Cryptography Rules
      {
        ...
        messageTemplate: 'Weak cryptographic algorithm detected. Use stronger alternatives like SHA-256 or AES.',
        remediation: 'Replace weak algorithms: Use SHA-256+ for hashing, AES for encryption.',
        cweId: 'CWE-327',
      ...
      // Authorization Rules
      {
        ...
        severity: 'high',
        pattern: /app\.(get|post|put|delete|patch)\(['"`][^'"`]*['"`]\s*,\s*(?!.*auth|.*login|.*public)[^,]*\s*,\s*(?:async\s+)?\([^)]*\)\s*=>/gi,
        messageTemplate: 'API endpoint may lack authorization checks. Ensure proper access control.',
        remediation: 'Add authorization middleware to verify user permissions before processing requests.',
        cweId: 'CWE-862',
      ...
      // Configuration Rules
      {
        ...
        description: 'Debug mode enabled which may expose sensitive information',
        category: 'configuration',
        severity: 'medium',
        pattern: /debug\s*[:=]\s*true|DEBUG\s*=\s*true|development/gi,
        messageTemplate: 'Debug mode detected. Ensure debug is disabled in production.',
        remediation: 'Disable debug mode in production environments and use proper logging instead.',
        cweId: 'CWE-489',
  ...
  private async loadSecretPatterns(): Promise<void> {
    const patterns: SecretPattern[] = [
      ...
      },
      {
        ...
        pattern: /(mysql|postgresql|mongodb):\/\/[^:\s]+:[^@\s]+@[^:\s]+:[0-9]+\/[^\s]+/gi,
  ...
  async scanProject(config: ScanConfiguration): Promise<string> {
    ...
    try {
      ...
      const files = await this.getFilesToScan(config.projectPath, config.includePatterns, config.excludePatterns);
  ...
  private async scanFile(
          ...
          for (const match of matches) {
            if (match.index !== undefined) {
              ...
              vulnerabilities.push({
                ...
                confidence: this.calculateConfidence(rule, match[0], lineContent),
    ...
    if (config.scanTypes.some(t => t.type === 'secret' && t.enabled)) {
      for (const [patternName, secretPattern] of this.secretPatterns.entries()) {
  ...
  private calculateConfidence(rule: SecurityRule, match: string, context: string): 'low' | 'medium' | 'high' {
  ...
  private calculateLikelihood(category: string, filePath: string): string {
    ...
    return likelihoods[category as keyof typeof likelihoods] || 'Medium - Standard risk';
  }
  ...
  private generateSummary(vulnerabilities: Vulnerability[], scannedFiles: number): ScanSummary {
  ...
  async generateReport(scanId: string, format: 'json' | 'sarif' | 'html' | 'csv' = 'json'): Promise<string> {
          ...
          tool: {
            driver: {
              ...
              rules: Array.from(this.securityRules.values()).map(rule => ({
                ...
                defaultConfiguration: { level: rule.severity === 'critical' ? 'error' : 'warning' },
                properties: {
                  category: rule.category,
                  cwe: rule.cweId,
...
<body>
  ...
  private generateCsvReport(result: ScanResult): string {
    const headers = ['ID', 'Severity', 'Category', 'Title', 'File', 'Line', 'Description', 'Remediation'];
